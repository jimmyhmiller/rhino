...
public class Compiler {
    ...
    private CompilerData.Builder<?> builder;
    private org.mozilla.javascript.JSDescriptor.Builder<?> descBuilder;
    ...
    public org.mozilla.javascript.JSDescriptor.Builder<?> getDescriptorBuilder() {
        return descBuilder;
    }

    public CompilerData<?> compile(
        ...
        descBuilder = new org.mozilla.javascript.JSDescriptor.Builder<>();
        CompilerData.Builder<?> cb =
        ...
        org.mozilla.javascript.JSCode.Builder codeBuilder = cb;
        descBuilder.code = codeBuilder;

        if (returnFunction) {
            @SuppressWarnings({"unchecked", "rawtypes"})
            org.mozilla.javascript.JSDescriptor.Builder fnBuilder = descBuilder;
            org.mozilla.javascript.CodeGenUtils.fillInForTopLevelFunction(
                    fnBuilder, (FunctionNode) scriptOrFn, rawSource, compilerEnv);
            org.mozilla.javascript.CodeGenUtils.setConstructor(descBuilder, scriptOrFn);
            generateFunctionCode();
        } else {
            org.mozilla.javascript.CodeGenUtils.fillInForScript(
                    descBuilder, scriptOrFn, rawSource, compilerEnv);
            org.mozilla.javascript.CodeGenUtils.setConstructor(descBuilder, scriptOrFn);
    ...
    private void generateNestedFunctions() {
        ...
        CompilerData<org.mozilla.javascript.JSFunction>[] array = new CompilerData[functionCount];
        for (int i = 0; i != functionCount; i++) {
            ...
            gen.scriptOrFn = fn;
            @SuppressWarnings("unchecked")
            CompilerData.Builder<org.mozilla.javascript.JSFunction> childCodeBuilder =
                    (CompilerData.Builder<org.mozilla.javascript.JSFunction>)
                            new CompilerData.Builder<>(this.builder);
            gen.builder = childCodeBuilder;
            @SuppressWarnings("unchecked")
            org.mozilla.javascript.JSDescriptor.Builder<org.mozilla.javascript.JSFunction>
                    childBuilder =
                            (org.mozilla.javascript.JSDescriptor.Builder<
                                            org.mozilla.javascript.JSFunction>)
                                    descBuilder.createChildBuilder();
            gen.descBuilder = childBuilder;
            childBuilder.code = childCodeBuilder;
            org.mozilla.javascript.CodeGenUtils.fillInForNestedFunction(
                    childBuilder, descBuilder, fn);
            org.mozilla.javascript.CodeGenUtils.setConstructor(childBuilder, fn);
            // TODO(Cam): If the function can't compile, we need to use old interpreter for that
            // function only.
            gen.generateFunctionCode();
            array[i] = childCodeBuilder.build();
